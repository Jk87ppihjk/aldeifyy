// ! Arquivo: adminRoutes.js (COM ROTA PÚBLICA DE BAIRROS E ATRIBUTOS AVANÇADOS)
...
// ROTA PÚBLICA 4: LISTAR ATRIBUTOS POR SUBCATEGORIA ID (CORRIGIDA)
router.get('/attributes/:subcategoryId', async (req, res) => {
    const subcategoryId = req.params.subcategoryId;
    try {
        // CORREÇÃO: Adiciona 'options' ao SELECT
        const [attributes] = await pool.execute(
            'SELECT id, name, type, options FROM attributes WHERE subcategory_id = ? ORDER BY name', 
            [subcategoryId]
        );
...
// ROTA PÚBLICA 5: LISTAR BAIRROS POR ID DA CIDADE
...
// 9. CRIAR Atributo (POST /api/admin/attributes) - CORRIGIDA
router.post('/admin/attributes', protectAdmin, async (req, res) => {
    // CORREÇÃO: Adiciona 'options' ao payload
    const { name, type, subcategory_id, options } = req.body; 
    
    if (!name || !type || !subcategory_id) { return res.status(400).json({ success: false, message: 'Nome, tipo e subcategoria são obrigatórios.' }); }

    try {
        // CORREÇÃO: Adiciona 'options' na query
        const [result] = await pool.execute(
            'INSERT INTO attributes (name, type, subcategory_id, options) VALUES (?, ?, ?, ?)', 
            [name, type, subcategory_id, options || null] // Usar || null para garantir que 'undefined' vire NULL
        );
...
// 10. BUSCAR TODOS os Atributos - (CORRIGIDA - para selecionar 'options')
router.get('/admin/attributes', protectAdmin, async (req, res) => {
    try {
        const [attributes] = await pool.execute(
            `SELECT a.id, a.name, a.type, a.subcategory_id, a.options, s.name AS subcategory_name, c.name AS category_name
             FROM attributes a
...
// 12. ATUALIZAR Atributo - CORRIGIDA
router.put('/admin/attributes/:id', protectAdmin, async (req, res) => {
    ...
    try {
        // CORREÇÃO: Adiciona 'options' na query
        const [result] = await pool.execute(
            'UPDATE attributes SET name = ?, type = ?, subcategory_id = ?, options = ? WHERE id = ?',
            [name, type, subcategory_id, options || null, attributeId]
        );
        if (result.affectedRows === 0) { return res.status(404).json({ success: false, message: 'Atributo não encontrado.' }); }
